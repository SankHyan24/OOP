...
void xml_test()
{
    const char *file_name = "./xml_test.xml";
    ...
    { // 基础测试——int/double/string/vector/set/list/map等基础类型
        ...
        ASSERT_STREQ(double_a, double_b);
    ...
    { // 高级测试——容器之间随意多重嵌套,甚至支持tuple
        ...
        for (size_t i = 0; i < v_t1.size(); i++)
        {
            ...
            ASSERT_STREQ(get<1>(v_t1[i]), get<1>(v_t2[i]));
    ...
    { // 用户自定义类型测试，需要在相应命名空间内使用 serialize_user/deserialize_user
        ...
        public:
            ...
            bool operator==(const my_class &rhs) const { return a == rhs.a && STR_EQ(b, rhs.b) && c == rhs.c; }
            int a;
            double b;
            string c;
        };
        ...
        struct my_struct
        {
            ...
            bool operator==(const my_struct &rhs) const { return a == rhs.a && STR_EQ(b, rhs.b) && c == rhs.c; }
            int a;
            double b;
            string c;
        };
